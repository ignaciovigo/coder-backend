...
import { redirectLoggedIn, redirectNotUser } from '../middlewares/authorization.js'
...
router.get('/products', redirectNotUser, async (req, res) => {
  try {
    ...
    const resultFormatted = { status: 'success', payload: docs, ...resp, prevLink, nextLink, ...req.session.user }
    res.render('products', { ...resultFormatted, style: '/css/index.css', scriptPath: '/js/products.js', scriptPath2: '/js/logout.js' })
  } catch (error) {
...
router.get('/carts/:cid', redirectNotUser, async (req, res) => {
  try {
    ...
    res.render('cart', { style: '/css/index.css', products: cart.products, totalPrice, scriptPath: '/js/cart.js', scriptPath2: '/js/logout.js', cartId: cid })
  } catch (error) {
    res.status(400).send({ status: 'error', message: error.message })
  }
})
