...
export default function ProductsContainer() {
  ...
  const handleClick = (e) => {
    ...
    if(type === 'prev') return getProducts({link: prevLink})
    if(type === 'next') return getProducts({link: nextLink})
  }
  return (
    <section className='h-full w-full flex flex-col gap-5 items-start py-4 px-3'>
      <InputSearch getProductsBySearch={getProductsBySearch} search={search} setSearch={setSearch}  />
